...
const makeTaskId = (): TaskId =>
  `task_${Date.now()}_${Math.random().toString(36).substr(2, 5)}` as TaskId;
const makeMessageId = (): MessageId =>
  `msg_${Date.now()}_${Math.random().toString(36).substr(2, 5)}` as MessageId;
...
class Agent {
  ...
  private handleHandoff(msg: Message): void {
    ...
    this.state.output = `Handed off to ${
      nextAgentId.split("_")[1]
    }. Waiting...`;
    this.notifyStateChange();
...
export default function AgentPollingDemo() {
  ...
  const [scenario, setScenario] = useState<
    "sequential" | "parallel" | "handoff" | "none"
  >("none");
  ...
  const runScenario = async (type: "sequential" | "parallel" | "handoff") => {
    ...
    if (type === "sequential") {
      ...
      coordinatorPolling.startPolling(
        ...
        messageBus,
        (status) => {
          /* handled by agent internally mostly */
        }
      );
  ...
  const getMessageStyle = (type: MessageType) => {
    switch (type) {
      ...
      case MessageType.STATUS_QUERY:
        return "border-l-4 border-slate-300 bg-slate-50 opacity-60"; // Dimmed
      case MessageType.STATUS_RESPONSE:
        return "border-l-4 border-amber-300 bg-amber-50 opacity-60"; // Dimmed
      case MessageType.HANDOFF:
        ...
        <div className="lg:col-span-12 flex flex-col md:flex-row items-start md:items-center justify-between bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-xl">
          <div className="mb-4 md:mb-0">
            ...
            <p className="text-slate-400 mt-2">
              Visualizing async message passing and status polling between
              autonomous agents.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${
                scenario === "sequential"
                  ? "ring-2 ring-blue-500 bg-blue-600/20 text-blue-400"
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${
                scenario === "parallel"
                  ? "ring-2 ring-purple-500 bg-purple-600/20 text-purple-400"
                  : "bg-slate-700 hover:bg-slate-600 text-white"
              } disabled:opacity-50 disabled:cursor-not-allowed`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${
                scenario === "handoff"
                  ? "ring-2 ring-emerald-500 bg-emerald-600/20 text-emerald-400"
                  : "bg-slate-700 hover:bg-slate-600 text-white"
              } disabled:opacity-50 disabled:cursor-not-allowed`}
            >
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="grid gap-4">
            {Array.from(agents.values()).map((agent) => (
              <div
                key={agent.id}
                className={`relative overflow-hidden rounded-xl border-2 p-5 transition-all duration-300 ${getStatusColor(
                  agent.status
                )}`}
              >
          ...
          >
            {messages.length === 0 ? (
              ...
              messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`p-3 rounded border-l-4 text-slate-800 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300 ${getMessageStyle(
                    msg.type
                  )}`}
                >
